...
export const Route = createFileRoute("/_public/marketplace/products/$slug")({
  loader: async ({ params, context }) => {
    const [product, reviewsData, hasUsedResult] = await Promise.all([
      serverApi.products.getBySlug({ slug: params.slug }),
      serverApi.products.getReviews({ slug: params.slug }),
      context.session
        ? serverApi.products.hasUsedProduct({ slug: params.slug })
    ...
    return {
      ...
      reviewsData,
      slug: params.slug,
      hasUsedProduct: hasUsedResult.hasUsed,
    }
  },
  head: ({ loaderData }) => {
...
function ProductDetailPage() {
  const { product, reviewsData, slug, hasUsedProduct } = Route.useLoaderData()
