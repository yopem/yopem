  ...
  switch (action.type) {
    ...
    case "LOAD_SUCCESS":
      return {
        ...state,
        isLoading: false,
        hasUsedProduct: action.payload.hasUsed,
...
const ProductRatingInput = ({ slug, onSuccess }: ProductRatingInputProps) => {
  const [
    {
      rating,
      hoverRating,
    ...
    const loadReviewData = async () => {
      try {
        const [reviewResult, usageResult] = await Promise.all([
          clientApi.products.getUserReview({ slug }),
          clientApi.products.hasUsedProduct({ slug }),
        ])
  ...
  const handleSubmit = async () => {
    ...
    try {
      await clientApi.products.submitReview({
        slug,
        rating,
