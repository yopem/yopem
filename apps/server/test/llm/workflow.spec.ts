...
test("condition does not execute injected code from variable values", async () => {
  const injectedWorkflow: ProductWorkflow = {
    nodes: [
      {
        id: "input_1",
        type: "input",
        ...
        data: {
          label: "Input",
          fields: [
            { variableName: "topic", description: "Topic", type: "text" },
          ],
        },
      },
      {
        ...
        type: "condition",
        position: { x: 200, y: 0 },
        data: { label: "Condition", expression: "'{{topic}}' == \"a\"" },
      },
      {
        id: "out_true",
        type: "output",
    ...
    edges: [
      { id: "e1", source: "input_1", target: "cond_1" },
      { id: "e2", source: "cond_1", target: "out_true", sourceHandle: "true" },
    ],
  }

  const result = await executeWorkflow({
    workflow: injectedWorkflow,
    inputs: { topic: "a'); (() => { throw new Error('pwned') })(); //" },
    productConfig: { modelEngine: "" },
    productApiKeyId: null,
    apiKeys: [],
  })
...
test("skips nodes downstream of a deactivated branch", async () => {
  const branchedWorkflow: ProductWorkflow = {
    nodes: [
      {
        id: "input_1",
        type: "input",
    ...
    edges: [
      ...
      { id: "e3", source: "out_false", target: "out_false_child" },
    ],
  }
  ...
  expect(result.steps.some((step) => step.nodeId === "out_false")).toBe(false)
  expect(result.steps.some((step) => step.nodeId === "out_false_child")).toBe(
    false,
  )
})
